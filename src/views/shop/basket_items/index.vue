...
export default {
  ...
  methods: {
    getList() {
      ...
      basketItem.getBasketItems(this.listQuery).then(response => {
      ...
      switch (command.type) {
        ...
        case 'del':
          this.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
            ...
            basketItem.deleteBasketItem(command.row.id).then((result) => {
              if (result.success) {
                this.getList()
