<template>
  <el-dialog :title="title" :visible.sync="dialogVisible" :close-on-click-modal="false" @closed="closeHandle">
    <el-form ref="editForm" :model="editForm" status-icon label-width="100px" class="lable-same-line">
      ...
      <el-form-item label="RefundPaymentMethod" prop="refundPaymentMethod" verify>
        <el-input v-model="editForm.refundPaymentMethod" autocomplete="off" />
      </el-form-item>
      ...
      <el-form-item label="DisplayReason" prop="displayReason" verify>
        <el-input v-model="editForm.displayReason" autocomplete="off" />
      </el-form-item>
      ...
      <el-form-item label="CompletedTime" prop="completedTime" verify>
        <el-date-picker
          v-model="editForm.completedTime"
          type="date"
          placeholder="选择日期"
          format="yyyy-MM-dd"
...
<script>
import { refund } from '@/api/module/eshop'
export default {
  name: 'RefundsEdit',
  data() {
    return {
  ...
  methods: {
    loadData(id) {
      ...
      refund.getRefund(id).then((result) => {
        loading.close()
        // 显示dialog
      ...
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.editForm.id !== undefined) {
            refund.updateRefund(this.editForm)
              .then((result) => {
                loading.close()
            ...
            refund.createRefund(this.editForm)
              .then((result) => {
                loading.close()
                this.$emit('save-success')
                this.$message.success('保存成功')
