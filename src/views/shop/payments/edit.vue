<template>
  <el-dialog :title="title" :visible.sync="dialogVisible" :close-on-click-modal="false" @closed="closeHandle">
    <el-form ref="editForm" :model="editForm" status-icon label-width="100px" class="lable-same-line">
      ...
      <el-form-item label="PaymentMethod" prop="paymentMethod" verify>
        <el-input v-model="editForm.paymentMethod" autocomplete="off" />
      </el-form-item>
      <el-form-item label="PayeeAccount" prop="payeeAccount" verify>
        <el-input v-model="editForm.payeeAccount" autocomplete="off" />
      </el-form-item>
      <el-form-item label="ExternalTradingCode" prop="externalTradingCode" verify>
        <el-input v-model="editForm.externalTradingCode" autocomplete="off" />
      </el-form-item>
      <el-form-item label="Currency" prop="currency" verify>
        <el-input v-model="editForm.currency" autocomplete="off" />
      </el-form-item>
      <el-form-item label="OriginalPaymentAmount" prop="originalPaymentAmount" verify>
        <el-input v-model="editForm.originalPaymentAmount" autocomplete="off" />
      </el-form-item>
      <el-form-item label="PaymentDiscount" prop="paymentDiscount" verify>
        <el-input v-model="editForm.paymentDiscount" autocomplete="off" />
      </el-form-item>
      <el-form-item label="ActualPaymentAmount" prop="actualPaymentAmount" verify>
        <el-input v-model="editForm.actualPaymentAmount" autocomplete="off" />
      </el-form-item>
      <el-form-item label="RefundAmount" prop="refundAmount" verify>
        <el-input v-model="editForm.refundAmount" autocomplete="off" />
      </el-form-item>
      <el-form-item label="PendingRefundAmount" prop="pendingRefundAmount" verify>
        <el-input v-model="editForm.pendingRefundAmount" autocomplete="off" />
      </el-form-item>
...
<script>
import { payment } from '@/api/module/eshop'
export default {
  name: 'PaymentsEdit',
  data() {
    return {
  ...
  methods: {
    loadData(id) {
      ...
      payment.getPayment(id).then((result) => {
        loading.close()
        // 显示dialog
      ...
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.editForm.id !== undefined) {
            payment.updatePayment(this.editForm)
              .then((result) => {
                loading.close()
            ...
            payment.createPayment(this.editForm)
              .then((result) => {
                loading.close()
                this.$emit('save-success')
                this.$message.success('保存成功')
