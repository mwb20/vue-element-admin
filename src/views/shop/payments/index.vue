...
export default {
  ...
  data() {
    return {
      ...
      cols: [
        ...
        {
          key: 'originalPaymentAmount',
          label: 'OriginalPaymentAmount',
          sortable: true,
          width: '100',
          showTooltip: true,
          show: true
  ...
  methods: {
    getList() {
      ...
      payment.getPayments(this.listQuery).then(response => {
      ...
      switch (command.type) {
        ...
        case 'del':
          this.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
            ...
            payment.deletePayment(command.row.id).then((result) => {
              if (result.success) {
                this.getList()
